import streamlit as st
#import streamlit.components.v1 as components
'''
st.header("Movies Links")
st.write("Bharateeyudu 2 -[Bharateeyudu 2](https://www.5movierulz.best/bharateeyudu-2-2024-telugu/movie-watch-online-free-2966.html)")
st.write("PUSHPA 2 - [PUSHPA](https://day.ibomma.observer/ca-tex4c/pushpa-the-rule-part-2-2024-r2tdf-telugu-movie-watch-online.html)")
st.write("I bomma Link - [ibomma](https://rta.bappam.fans/telugu-movies/)")
st.write(" Lucky Baskhar - [Lucky Baskhar](https://www.5movierulz.io/lucky-baskhar-2024-telugu/movie-watch-online-free-3649.html)")
st.write(" Movierulz - [Amaran] (https://rta.bappam.fans/ai-n2cdl/amaran-2024-grm4p-telugu-movie-watch-online.html)")
'''
#html_string = "<div style="padding:42.5% 0 0 0;position:relative;"><iframe src="https://player.vimeo.com/video/1050532555?badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479" frameborder="0" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media" style="position:absolute;top:0;left:0;width:100%;height:100%;" title="Sankranthiki Vasthunam"></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>"
st.markdown(
'<iframe src="https://player.vimeo.com/video/1057564424?h=f3c2d5274a&amp;title=0&amp;byline=0&amp;portrait=0&amp;badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479" width="500" height="204" frameborder="0" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media" title="www.MoviezWap.diy - Thandel (2025) Telugu HDTS 720p HQ"></iframe>',unsafe_allow_html=True)
#components.iframe("https://www.youtube.com", width=800, height=600) 
#components.iframe("https://www.5movierulz.rip/daaku-maharaaj-2025-telugu/movie-watch-online-free-4064.html", width=800, height=600) 
#<div style="padding:42.5% 0 0 0;position:relative;"><iframe src="https://player.vimeo.com/video/1050532555?badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479" frameborder="0" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media" style="position:absolute;top:0;left:0;width:100%;height:100%;" title="Sankranthiki Vasthunam"></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>
